...
def cmd_delete_inbox(args):
    ...
    if not args.force:
        response = input(
            "Are you sure you want to delete your entire inbox? This cannot be undone! (yes/no): "
        )
...
def cmd_config(args):
    ...
    if args.api_url:
        ...
        config.save()
        print_success("API URL saved to config file")
        return 0
...
def create_parser() -> argparse.ArgumentParser:
    ...
    # Global options
    parser.add_argument(
        "--api-url", default=None, help="API base URL (default: https://api.sinkron.id)"
    )
    ...
    register_parser.add_argument(
        "--username", required=True, help="Username (4-25 lowercase alphanumeric)"
    )
    register_parser.add_argument("--name", required=True, help="Display name")
    register_parser.add_argument(
        "--save-token", action="store_true", help="Save token to config file"
    )
    ...
    delete_msgs_parser.add_argument(
        "--ids", required=True, help="Comma-separated message IDs (max 25)"
    )
